...
3. The code should be modular. The code should have the basic OO design. Please do not jamz in
...
func main() {
	...
	rider := RiderConstracter("Sheelendar")

	rider.createRide(1, 50, 60, 1)
	fmt.Println("Ride is finished fare is ", rider.closeRide())
	rider.updateRide(1, 50, 60, 1)
	fmt.Println()

	rider.createRide(2, 50, 60, 1)
	rider.withdrowRide(2)
	rider.updateRide(2, 50, 60, 1)
	fmt.Println("Ride is finished fare is ", rider.closeRide())
	fmt.Println()

	rider.createRide(3, 50, 60, 1)
	rider.updateRide(3, 50, 60, 2)
	fmt.Println("Ride is finished fare is ", rider.closeRide())
	fmt.Println(rider.Rides)
}
